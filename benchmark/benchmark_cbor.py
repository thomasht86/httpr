...
async def benchmark_cbor_vs_json():
    ...
    for data_count in [1, 10, 100]:
        for gzip_param in ["false", "true"]:
            json_result = next(r for r in results if r["format"] == "JSON" and r["count"] == data_count and r["gzip"] == gzip_param)
            cbor_result = next(r for r in results if r["format"] == "CBOR" and r["count"] == data_count and r["gzip"] == gzip_param)
