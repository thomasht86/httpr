...
`client.headers` is a live, mutable view — in-place changes are applied to the
client and included in subsequent requests:
...
- The timeout bounds how long the server may keep you waiting, not how long
  a request may take: it applies to the wait for the response headers and then
  to the wait for each chunk of the body, and every chunk that arrives resets it.
  A large download or a long-lived stream that keeps delivering data is never
  cut off; a server that goes silent for longer than the timeout is
