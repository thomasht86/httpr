...
Available helpers: `is_informational` (1xx), `is_success` (2xx), `is_redirect` (3xx), `is_client_error` (4xx), `is_server_error` (5xx), `is_error` (4xx/5xx), and `has_redirect_location` (3xx with a `Location` header).
...
The simplest way to turn a non-2xx response into an exception is `raise_for_status()`. It raises `HTTPStatusError` for any non-2xx status and otherwise returns the response, so it can be chained:
...
Unlike `requests`, `raise_for_status()` treats **any** non-2xx response as an error (including 1xx and 3xx), matching `httpx`. If you only want to act on server/client errors, check `response.is_error` instead.
