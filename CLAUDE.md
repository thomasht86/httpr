...
- Connection tasks live on the single-threaded `RUNTIME` and only progress inside `block_on`, so dropping a pool does not close sockets by itself: `settle_dropped_pool()` runs a fixed number of `yield_now` turns so the connection tasks observe the hang-up and send FIN (deterministic, well under a millisecond, never waits on I/O). If another thread holds the scheduler core (it is mid-request), its scheduler closes them instead
